...
### <span style="color:#F5B1C8">📄 pom.xml</span>
...
단, 본인이 사용하는 spring과 JPA의 하이버네이트와 본인이 사용하는 h2 버전을 <span style="color:#BADADF">통일 시키는 것이 좋다.</span>

### <span style="color:#F5B1C8">📄 persistence.xml</span>
...
<span style="color:#BADADF">JPA는 특정 데이터 베이스에 종속적이지 않다.</span>
따라서 데이터 베이스 방언 클래스(Dialect)를 제공하는데 프로젝트 개발자는 이에 대하여 사용하는 데이터 베이스가 변경되어도 <span style="color:#BADADF">방언만 교체하면 쉽게 DB를 변경할 수 있다.</span>
...
<span style="color:#F5B1C8">📄 Member.java</span>
...
<span style="color:#F5B1C8">📄 JpaMain.java</span>
...
- <span style="color:#BADADF">JPA의 모든 데이터 변경은 트랜잭션 안에서 작업해야 한다.</span>
